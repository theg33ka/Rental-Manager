...
Run-Native $Javac (@("-encoding", "UTF-8", "-source", "8", "-target", "8", "-bootclasspath", $AndroidJar, "-cp", "$Root\build\gen", "-d", "$Root\build\classes") + $Sources)
...
if (!(Test-Path "$Root\build\debug.keystore")) {
    Run-Native $Keytool @("-genkeypair", "-v", "-keystore", "$Root\build\debug.keystore", "-storepass", "android", "-keypass", "android", "-alias", "androiddebugkey", "-keyalg", "RSA", "-keysize", "2048", "-validity", "10000", "-dname", "CN=Android Debug,O=Rental Manager,C=RU")
}

Run-Native "$BuildTools\apksigner.bat" @("sign", "--ks", "$Root\build\debug.keystore", "--ks-pass", "pass:android", "--key-pass", "pass:android", "--out", "$Root\build\rental-manager-mobile.apk", "$Root\build\rental-manager-mobile-aligned.apk")
