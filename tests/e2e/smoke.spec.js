...
test("дашборд группирует объекты и сворачивает однотипные долги", async ({ page }) => {
  const result = await page.evaluate(() => {
    ...
    const utilityItem = (id, debt, period) => ({
      id,
      lease_id: 10,
      object: "Баня",
      apartment: "Баня 3",
    ...
    const dashboard = {
      rent_overdue: [rentItem(1, "2026-05-05"), rentItem(2, "2026-06-05")],
      rent_partial: [],
      rent_today: [],
      rent_deferred: [],
      utility_overdue: [utilityItem(3, 5000, "01.04 → 01.05"), utilityItem(4, 7000, "01.05 → 01.06")],
      utility_partial: [],
      utility_issued: [],
      manual_debts: [],
    };
    const group = collectTenantAttentionGroups(dashboard)[0];
    const cardHtml = tenantAttentionCard(group);
    const sectionsHtml = renderDashboardAttentionSections(
      [
        { object: "Белый дом", html: "<article>БД2</article>" },
        { object: "Баня", html: cardHtml },
        { object: "Баня", html: "<article>Баня 4</article>" },
      ],
      ["Баня", "Белый дом"],
    );
    ...
    return {
      cardText: new DOMParser().parseFromString(cardHtml, "text/html").body.textContent.replace(/\s+/g, " ").trim(),
      sectionNames: [...host.querySelectorAll(".attention-object-group__head h3")].map((item) => item.textContent),
      firstSectionCards: host.querySelectorAll(".attention-object-grid")[0].children.length,
    };
  });
  ...
  expect(result.cardText).toContain("12 000");
  expect(result.cardText).not.toContain("01.04 → 01.05");
  expect(result.sectionNames).toEqual(["Баня", "Белый дом"]);
  expect(result.firstSectionCards).toBe(2);
});
