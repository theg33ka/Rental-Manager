...
Web «Коммуналка» → «Календарь оплат»: отдельное окно с горизонтальной шкалой дней, динамическими объектами/квартирами, переключением аренды и коммуналки и деталями дня. Backend `/api/utilities/calendar` читает договоры и финансовые строки без пересчёта; новые объекты не требуют изменений frontend. Цвет показывает текущую оплату периода, а не исторический снимок на выбранную дату.
