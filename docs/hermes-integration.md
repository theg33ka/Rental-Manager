...
Write-инструмент `propose_owner_operation` формирует предложение одной из разрешённых
операций веб-пульта. Каталог находится в
`rental_manager/services/owner_operations.py` и включает управление объектами,
квартирами, договорами, платежами, долгами, коммуналкой, показаниями, тарифами,
расходами, отчётами, сообщениями и напоминаниями.

Далее backend:

1. валидирует реальные `lease_id`, `charge_id`, даты, суммы и Telegram-привязку;
2. создаёт pending action;
3. показывает владельцу точные параметры, порядок выполнения и кнопки
   `Подтвердить` / `Отклонить` в том же сообщении;
...
Повторное нажатие не выполняет действие дважды. Текстовые ответы `да`, `ок`,
`выполняй` и похожие никогда не считаются подтверждением.

Кнопка `Открыть пульт` не добавляется к обычным ответам. Она появляется только
по команде `/app` и в стартовом сообщении; для отчётов выводятся только ссылки
на сами отчёты.
...
6. Написать: `Подготовь сообщение должнику` — должна появиться подробная карточка подтверждения.
