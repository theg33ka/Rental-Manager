...
| Просмотреть календарь оплат | owner; Web «Коммуналка» → «Календарь оплат» | Jump to month, rent/utility mode, day, zoom | Dynamic objects/apartments on page without inner vertical scrolling; lazy horizontal history with bounded cache; cursor-anchored wheel zoom and slider/reset; exact invoice bands/tooltips; recent unbilled, aged unbilled and gaps distinguished; hidden overdue jump; archived rows retained | `GET /api/utilities/calendar` + `/summary`; read-only; no historic as-of reconstruction |
